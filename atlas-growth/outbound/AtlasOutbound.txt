...
// Google Apps Script da planilha de contatos.
//
// ESTE DEVE SER O ÚNICO ARQUIVO .gs DO PROJETO. No Apps Script todos os
// arquivos dividem o mesmo escopo global: um segundo arquivo declarando
// CFG, onOpen ou planilha_ sobrescreve o que está aqui, e o menu que
// aparece passa a ser o do outro. Apague os demais antes de usar.
//
...
function montarFollowUp_(numero, nome) {
  var assunto = numero === 2 ? CFG.ASSUNTO_RASCUNHO_2 : CFG.ASSUNTO_RASCUNHO_3;
  var tpl = pegarTemplate_(assunto, false);

  if (tpl) {
    var html = tpl.html.replace(/\{\{nome\}\}/g, nome);
    return { html: html, texto: htmlParaTexto_(html) };
  }

  var texto = numero === 2 ? followUp1_(nome) : encerramento_(nome);
...
function pegarTemplate_(assunto, obrigatorio) {
  var alvo = (assunto || CFG.ASSUNTO_RASCUNHO).trim();
...
function conferirLinks() {
  ...
  var relatorio = partes.map(function (parte) {
    var tpl = pegarTemplate_(parte.assunto, parte.obrigatorio);
    if (!tpl) return parte.rotulo + ': sem rascunho — usa o texto puro do código';
    var links = (tpl.html.match(/href="([^"]+)"/g) || []).map(function (h) {
      return '   ' + h.replace('href="', '').replace('"', '');
    });
    var sujo = tpl.html.indexOf('google.com/url') !== -1;
